 ...
 */
export async function webhooksVerifyEvent(
  client$: PanoraCore,
...
> {
  ...
  const parsed$ = schemas$.safeParse(
    input$,
    (value$) =>
      components.SignatureVerificationDto$outboundSchema.parse(value$),
  ...
  const [result$] = await m$.match<
    ...
    | ConnectionError
  >(
    m$.json(201, components.EventPayload$inboundSchema),
    m$.fail(["4XX", "5XX"]),
  )(response);
  if (!result$.ok) {
    return result$;
