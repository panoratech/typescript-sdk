  ...
  describe('test updateWebhookStatus', () => {
    test('test api call', () => {
      const scope = nock('https://api-demo.panora.dev')
        .put('/webhook/6714390659')
        .reply(200, { data: {} });
      return sdk.webhook
        .updateWebhookStatus('6714390659')
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-demo.panora.dev')
        .put('/webhook/2177462304')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-demo.panora.dev')
        .put('/webhook/1672589927')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.webhook.updateWebhookStatus('1672589927'),
      ).rejects.toThrow();
    });
  });
});
