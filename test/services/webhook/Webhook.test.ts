  ...
  describe('test updateWebhookStatus', () => {
    test('test api call', () => {
      const scope = nock('https://api-demo.panora.dev')
        .put('/webhook/6971984758')
        .reply(200, { data: {} });
      return sdk.webhook
        .updateWebhookStatus('6971984758')
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-demo.panora.dev')
        .put('/webhook/9167577924')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-demo.panora.dev')
        .put('/webhook/4352411008')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.webhook.updateWebhookStatus('4352411008'),
      ).rejects.toThrow();
    });
  });
});
