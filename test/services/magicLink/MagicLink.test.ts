...
describe('test MagicLink', () => {
  ...
  describe('test getMagicLink', () => {
    test('test api call', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/magic-link/single?id=1609551103')
        .reply(200, { data: {} });
      return sdk.magicLink.getMagicLink('1609551103').then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/magic-link/single?id=7112122133')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/magic-link/single?id=4950111921')
        .reply(404, { data: {} });
      return expect(async () => await sdk.magicLink.getMagicLink('4950111921')).rejects.toThrow();
    });
  });
});
