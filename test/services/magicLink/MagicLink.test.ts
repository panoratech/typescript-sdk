...
describe('test MagicLink', () => {
  ...
  describe('test getMagicLink', () => {
    test('test api call', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/magic-link/single?id=7198985460')
        .reply(200, { data: {} });
      return sdk.magicLink.getMagicLink('7198985460').then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/magic-link/single?id=8606549915')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/magic-link/single?id=5605031349')
        .reply(404, { data: {} });
      return expect(async () => await sdk.magicLink.getMagicLink('5605031349')).rejects.toThrow();
    });
  });
});
