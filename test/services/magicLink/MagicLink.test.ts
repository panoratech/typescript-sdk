...
describe('test MagicLink', () => {
  ...
  describe('test getMagicLink', () => {
    test('test api call', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/magic-link/single?id=6656939182')
        .reply(200, { data: {} });
      return sdk.magicLink.getMagicLink('6656939182').then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/magic-link/single?id=8787009198')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/magic-link/single?id=8939310326')
        .reply(404, { data: {} });
      return expect(async () => await sdk.magicLink.getMagicLink('8939310326')).rejects.toThrow();
    });
  });
});
