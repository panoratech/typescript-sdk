...
describe('test FieldMapping', () => {
  ...
  describe('test getCustomProviderProperties', () => {
    test('test api call', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/field-mapping/properties?linkedUserId=voluptatibus&providerId=8868128666')
        .reply(200, { data: {} });
      return sdk.fieldMapping
        .getCustomProviderProperties('voluptatibus', '8868128666')
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/field-mapping/properties?linkedUserId=natus&providerId=9384056097')
        .reply(200, { data: {} });
      return expect(
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/field-mapping/properties?linkedUserId=dolor&providerId=5640659080')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.fieldMapping.getCustomProviderProperties('dolor', '5640659080'),
      ).rejects.toThrow();
    });
  });
});
