  ...
  describe('test handleOAuthCallback', () => {
    test('test api call', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/connections/oauth/callback?state=ipsam&code=dignissimos&location=error')
        .reply(200, { data: {} });
      return sdk.connections
        .handleOAuthCallback('ipsam', 'dignissimos', 'error')
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/connections/oauth/callback?state=ipsam&code=libero&location=dolore')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/connections/oauth/callback?state=maiores&code=similique&location=sunt')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.connections.handleOAuthCallback('maiores', 'similique', 'sunt'),
      ).rejects.toThrow();
    });
  });
