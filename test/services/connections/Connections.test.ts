  ...
  describe('test handleOAuthCallback', () => {
    test('test api call', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/connections/oauth/callback?state=nemo&code=aliquam&location=molestias')
        .reply(200, { data: {} });
      return sdk.connections
        .handleOAuthCallback('nemo', 'aliquam', 'molestias')
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/connections/oauth/callback?state=debitis&code=sed&location=rem')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/connections/oauth/callback?state=voluptatem&code=voluptas&location=neque')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.connections.handleOAuthCallback('voluptatem', 'voluptas', 'neque'),
      ).rejects.toThrow();
    });
  });
