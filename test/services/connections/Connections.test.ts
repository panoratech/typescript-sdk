  ...
  describe('test handleOAuthCallback', () => {
    test('test api call', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/connections/oauth/callback?state=perferendis&code=odit&location=minima')
        .reply(200, { data: {} });
      return sdk.connections
        .handleOAuthCallback('perferendis', 'odit', 'minima')
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/connections/oauth/callback?state=sequi&code=esse&location=quo')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api-demo.panora.dev')
        .get('/connections/oauth/callback?state=fugit&code=modi&location=iste')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.connections.handleOAuthCallback('fugit', 'modi', 'iste'),
      ).rejects.toThrow();
    });
  });
